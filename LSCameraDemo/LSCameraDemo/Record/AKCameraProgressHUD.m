...
@implementation AKCameraProgressHUD
#pragma mark -
...
- (void)setMode:(AKCameraProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == AKCameraProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(AKRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == AKCameraProgressHUDModeDeterminate) {
#if __has_feature(objc_arc)
        self.indicator = [[AKRoundProgressView alloc] init];
#else
        self.indicator = [[[MBRoundProgressView alloc] init] autorelease];
#endif
    }
    else if (mode == AKCameraProgressHUDModeCustomView && self.customView != nil){
...
+ (AKCameraProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	AKCameraProgressHUD *hud = [[AKCameraProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[AKCameraProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		AKCameraProgressHUD *HUD = (AKCameraProgressHUD *)viewToRemove;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.mode = AKCameraProgressHUDModeDeterminate;
        self.labelText = nil;
...
@implementation AKRoundProgressView
